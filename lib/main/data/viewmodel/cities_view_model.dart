...
class CitiesViewModel extends StateNotifier<ConfigState> {
  final ClientProfileRepository repository;

  CitiesViewModel({required this.repository}) : super(ConfigInitial());

  Future<void> getAllCities(String countryId,String regionId) async {
    state = ConfigLoading();
    try {
      final clientProfile = await repository.getAllCities(countryId,regionId);
