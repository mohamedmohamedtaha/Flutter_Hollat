...
class RegionsViewModel extends StateNotifier<ConfigState> {
  final ClientProfileRepository repository;

  RegionsViewModel({required this.repository}) : super(ConfigInitial());

  Future<void> getAllRegions(String countryId) async {
    state = ConfigLoading();
    try {
      final clientProfile = await repository.getAllRegions(countryId);
