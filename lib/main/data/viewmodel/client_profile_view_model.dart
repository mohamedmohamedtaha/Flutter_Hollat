...
class ClientProfileViewModel extends StateNotifier<ConfigState> {
  final ClientProfileRepository repository;

  ClientProfileViewModel({required this.repository}) : super(ConfigInitial());

  Future<void> getClientProfile() async {
    state = ConfigLoading();
    try {
      final clientProfile = await repository.getClientProfile();
